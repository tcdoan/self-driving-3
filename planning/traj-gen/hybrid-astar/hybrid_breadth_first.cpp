...
double HBF::heuristic(double x, double y, const vector<int> &goal) {
    //return pow(goal[1]-x, 2) + pow(goal[0]-y, 2);
    return fabs(goal[1]-x) + fabs(goal[0]-y);
}
...
HBF::maze_path HBF::search(vector< vector<int> > &grid, vector<double> &start, 
  ...
  bool finished = false;
  while(!opened.empty()) {
    std::sort(opened.begin(), opened.end(), [](const maze_s& stateA, const maze_s& stateB) {
