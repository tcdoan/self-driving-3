    ...
    float position_stdev) 
{
  ...
  float uniformProb = 1.0 / (landmark_positions.size() * (1+2*position_stdev));
  for (int i = 0; i < landmark_positions.size(); i++) {
    for (int j = landmark_positions[i] - position_stdev; j <= landmark_positions[i] + position_stdev; j++) {
      priors[j] = uniformProb;
    }
  }
