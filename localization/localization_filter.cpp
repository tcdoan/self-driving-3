// Implement the Bayes's localization filter for multiple time steps and sensor observations.
//  Sensor observations are provided in a 2D vector where each inner vector represents
//  the sensor observations, in meters, at a time step.
//  {{1,7,12,21}, {0,6,11,20}, {5,10,19}, {4,9,18}, {3,8,17}, {2,7,16},
//  {1,6,15}, {0,5,14}, {4,13}, {3,12},{2,11},{1,10},{0,9},{8},{7},{6},{5},
//  {4},{3},{2},{1},{0}, {}, {}, {}};
//
// Implement the Bayes' localization filter by first initializing priors,
// then doing the following within each time step:
//      1. extract sensor observations
//          1.1. for each pseudo-position:
//              1.1.a get the motion model probability
//              1.1.b determine pseudo ranges
//              1.1.c get the observation model probability
//              1.1.d use the motion and observation model probabilities to calculate the posterior probability
//          1.2. normalize posteriors (see helpers.h for a normalization function)
//          1.3. update priors (priors --> posteriors)
    ...
    int control_stdev)
{
    ...
    for (float k = 0; k < map_size; ++k)
    {
        float prev_pseudo_position = k;

        // distance from k to j
        float distance_kj = pseudo_position - prev_pseudo_position;

        // transition probabilities:
        float transition_prob = Helpers::normpdf(distance_kj,
                                                 movement,
                                                 control_stdev);

        // estimate probability for the motion model, this is our prior
        position_prob += transition_prob * priors[k];
    }
    ...
    float observation_stdev)
{
    ...
    for (int z = 0; z < observations.size(); ++z)
    {
        ...
        distance_prob *= Helpers::normpdf(observations[z],
                                          pseudo_range_min,
                                          observation_stdev);
    }
...
int main()
{
    ...
    for (int t = 0; t < time_steps; ++t)
    {
        ...
        // step through each pseudo position x(t, k)
        for (unsigned int k = 0; k < map_size; ++k)
        {
            float pseudo_position = float(k);
            ...
            float posteriorProb = motion_prob * obs_prob;

            posteriors[k] = posteriorProb;            
        }
